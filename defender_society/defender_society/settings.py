...
# Email setting Email verification method during registration: one of "mandatory", "optional (default)" or "none". If
# email verification is enabled, an error will be reported if the email configuration is unavailable, so it is closed
# by default and turned on as needed
...
TEMPLATES = [
    {
        ...
        'OPTIONS': {
            ...
            # load templates from cache. Only works when debug = False
            # 'loaders': [
...
HAYSTACK_CONNECTIONS = {
    'default': {
        ...
        # Choose the language parser to replace the stuttering word segmentation for yourself
        'PATH': os.path.join(BASE_DIR, 'whoosh_index'),
        # Save the address of the index file, select the home directory, this will be automatically generated
    }
}
...
POSTGRES_NAME = os.getenv('defender_society_POSTGRES_NAME', 'bi7pimt')
POSTGRES_USER = os.getenv('defender_society_POSTGRES_USER', 'bi7pimt')
POSTGRES_PASSWORD = os.getenv('defender_society_POSTGRES_PASSWORD', 'test')
...
# persistent cached data written to db.
SESSION_ENGINE = 'django.contrib.sessions.backends.cached_db'


# Configure the management mailbox. If the service fails, you will receive an email. The format of the environment
# variable value: name|test@test.com Multiple groups of users are separated by English commas
ADMINS = []
...
EMAIL_HOST_PASSWORD = os.getenv('defender_society_EMAIL_HOST_PASSWORD',
                                'your-email-password')  # This is not an email password, but an authorization code
EMAIL_PORT = os.getenv('defender_society_EMAIL_PORT', 22)
EMAIL_TIMEOUT = 5
# Whether SSL or TLS is used, in order to use port 465, use this EMAIL_USE_SSL = os.getenv('IZONE_EMAIL_USE_SSL',
# 'True').upper() =='TRUE' The default sender, if you don’t set it, Django uses webmaster@localhost by default,
# so you should set it to your available mailbox
...
SITE_DESCRIPTION = os.getenv('defender_society_SITE_DESCRIPTION',
                             'Alumni Association blog for Big Brothers Big Sisters of Kentuckiana')
...
# Cache time to live is 45 minutes.
CACHE_TTL = 60 * 45
