...
def site_protocol():
    """
    Return the currently used protocol http|https,
     #which can be called to many places where the full address of the website is needed
...
def site_domain():
    ...
    SITE_ID needs to be configured in the settings configuration, and django.contrib.sites needs to be added in
    INSTALLED_APPS :return: current site domain name
    """
...
def get_tweets():
    c = twint.Config()
    c.Username = 'bbbskentuckiana'
    c.Pandas = True
    c.Since = '2021-04-10'
    twint.run.Search(c)
    tweets_df = twint.storage.panda.Tweets_df['tweet']
    content_tweets = list(tweets_df)
